...
function hierarchy(parent_id, act_dom_id) {
	...
	this.object_prefix = 'tree-' + data_loads;
	...
	this.make_load_more_html = function(item_id, node_i, index){
		...
		var tog_html = [
			'<span id="' + dom_id + '">',
			'<a title="Click to load more data and expand" ',
	...
	this.make_more_div_html = function(node_i, index){
		var more_div_dom_id = this.object_prefix + '-more-' + node_i + '-' +index;
		var html = [
		'<div id="' + more_div_dom_id + '" class="container collapse" style="margin-top:5px; margin-bottom:5px">',
		'</div>'
		].join('\n');
		return html;
	}
