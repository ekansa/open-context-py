...
def make_new_context_table_from_old_context_table(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA, alias='context'):
...
def make_first_temp_context_table(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA, alias='context'):
    ...
    SELECT
        ...
        man.context_uuid
    FROM man
    INNER JOIN {db_schema}.oc_all_manifest AS pm ON pm.uuid = man.context_uuid
    WHERE (man.item__geo_source IS NULL) OR (man.item__chrono_source IS NULL)
    """
    con.execute(f"DROP TABLE IF EXISTS {alias}")
    temp_table_sql = f'CREATE TEMPORARY TABLE {alias} AS {sql}'
    con.execute(temp_table_sql)
...
def update_sample_from_context_geo(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA):
    ...
    WHERE man.item__geo_source IS NULL
    AND man.uuid = context.uuid
...
def update_sample_from_context_chrono(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA):
    ...
    FROM {db_schema}.oc_all_spacetime AS spt
    INNER JOIN context ON context.context_uuid = spt.item_uuid
    WHERE man.item__chrono_source IS NULL
    AND man.uuid = context.uuid
...
def update_sample_specific_geo(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA):
    """Update the sample manifest with the geo information specific to a given sample record"""
    ...
    SET item__latitude = spt.latitude,
        ...
        item__geometry_type = spt.geometry_type,
        item__geo_specificity = spt.geo_specificity,
        item__geo_source = concat('Given for: ', man.label)
    FROM {db_schema}.oc_all_spacetime AS spt
    WHERE man.item__geo_source IS NULL
    AND man.uuid = spt.item_uuid
    AND spt.latitude IS NOT NULL
    AND spt.longitude IS NOT NULL
    """
    con.execute(sql)
...
def update_sample_specific_chrono(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA):
    """Update the sample manifest with the chrono information specific to a given sample record"""
    ...
    SET item__earliest = spt.earliest,
        item__latest = spt.latest,
        item__chrono_source = concat('Given for: ', man.label)
    FROM {db_schema}.oc_all_spacetime AS spt
    WHERE man.item__chrono_source IS NULL
    AND man.uuid = spt.item_uuid
    AND spt.earliest IS NOT NULL
...
def get_isamples_spacetime_values(con=DB_CON, db_schema=duckdb_con.DUCKDB_SCHEMA, show_progress=True):
