...
def make_isamples_manifest_query_sql(more_where_clauses=None, db_schema=duckdb_con.DUCKDB_SCHEMA):
    ...
    -- Context labels
    get_path_level_item(m.path, 0) AS context___1,
    get_path_level_item(m.path, 1) AS context___2,
    get_path_level_item(m.path, 2) AS context___3,
    get_path_level_item(m.path, 3) AS context___4,
    get_path_level_item(m.path, 4) AS context___5,
    get_path_level_item(m.path, 5) AS context___6,
...
def get_isamples_raw_manifest(more_where_clauses=None, con=DB_CON, alias='man'):
    ...
    db_m = con.sql(sql).set_alias(alias)
    db_m.show()
    return db_m
