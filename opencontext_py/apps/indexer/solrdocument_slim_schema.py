...
ALL_LINK_DATA_SOLR = 'obj_all_ld' + SOLR_VALUE_DELIM + FIELD_SUFFIX_PREDICATE
...
class SolrDocumentSlim:
    ...
    def _get_predicate_solr_field_name_in_hierarchy(
        ...
        if assert_dict.get('predicate__item_type') == 'predicates':
            ...
            all_obj_solr_field = ALL_PREDICATE_SOLR
        else:
            # This is a linked data predicate.
            # root_solr_field = ROOT_LINK_DATA_SOLR
            all_obj_solr_field  = ALL_LINK_DATA_SOLR


        all_obj_solr_field = self._prefix_solr_field(all_obj_solr_field)
        # Add the solr field if it doesn't exist.
        if not self.fields.get(all_obj_solr_field):
        ...
        for _, hierarchy_items in enumerate(hierarchy_paths):
            ...
            for index, item_obj in enumerate(hierarchy_items):
                ...
                # Compose the solr value for the current parent item.
                act_solr_predicate_value = solr_utils.make_solr_event_attribute_predicate_entity_str(
                    event_slug=assert_dict.get('event__slug', 'oc-default-event'),
                    attribute_slug=assert_dict.get('attribute_group__slug', 'oc-default-attrib-group'),
                    predicate_slug=item.get('slug'),
                ...
                solr_field_name = solr_utils.convert_slug_to_solr(
                    event_attribute_predicate_slug
                    + SOLR_VALUE_DELIM
