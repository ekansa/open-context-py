...
class SearchSolrSlim():
    ...
    def solr_connect(self):
        ...
        if configs.USE_TEST_SOLR_CONNECTION:
            ...
            self.solr = SolrClient(use_alt_collection=True, use_test_solr=True).solr
        else:
            # Connect to the default solr server
            self.solr = SolrClient(use_alt_collection=True, ).solr
    ...
    def compose_query(self, request_dict):
        ...
        # -------------------------------------------------------------
        embedding_str = None
        raw_vibe_search = utilities.get_request_param_value(
            request_dict,
            param='qv',
            default=None,
            as_list=False,
        ...
        if raw_vibe_search:
            # Generate a vector embedding of the search string.
            embedding_str = make_vectorized_embedding_query_str(
                str_to_vectorize=raw_vibe_search,
            )
            if embedding_str:
