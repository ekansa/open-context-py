...
def get_discovery_geotile_query_dict(raw_disc_geo, low_res=True):
    ...
    return make_tile_query_dict(
        ...
        max_path_length=SolrDocSlim.MAX_GEOTILE_ZOOM,
    )
...
def get_spatial_context_query_dict(spatial_context=None):
    ...
    if not spatial_context:
        query_dict['fq'] = []
        query_dict['facet.field'] = [SolrDocSlim.ROOT_CONTEXT_SOLR]
        return query_dict
    ...
    for man_obj in db_entities.get_cache_manifest_items_by_path(
        ...
        path_term = utilities.make_solr_term_via_slugs(
            ...
            solr_dyn_field=SolrDocSlim.FIELD_SUFFIX_CONTEXT,
        ...
        query_dict['facet.field'].append(
            ...
            + SolrDocSlim.FIELD_SUFFIX_CONTEXT
        )
...
def get_general_hierarchic_path_query_dict(
    ...
    for path_index, item_id in enumerate(path_list):
        ...
        if item_id.startswith(configs.RELATED_ENTITY_ID_PREFIX):
            ...
            use_solr_rel_prefix = SolrDocSlim.RELATED_SOLR_DOC_PREFIX
        ...
        if (item_obj.item_type in URI_ITEM_TYPES
           and facet_field == SolrDocSlim.ROOT_PREDICATE_SOLR):
            facet_field = SolrDocSlim.ROOT_LINK_DATA_SOLR
        ...
        # NOTE: If SolrDocSlim.DO_LEGACY_FQ, we're doing the older
        ...
        if (item_obj.item_type in ['predicates', 'property']
            ...
            if attribute_item_obj.data_type in configs.LITERAL_DATA_TYPES:
                ...
                if not attribute_item_obj.meta_json.get('solr_field'):
                    ...
                    solr_parts = attribute_item_obj.meta_json.get('solr_field').split(SolrDocSlim.SOLR_VALUE_DELIM)
                    use_parts = SolrDocSlim.SOLR_VALUE_DELIM.join(solr_parts[:-1])
                    attribute_field_part = (
                        use_parts
                        + SolrDocSlim.SOLR_VALUE_DELIM
                    )
...
def get_linked_dinaa_query_dict():
    ...
    query_dict['fq'].append(f'{SolrDocSlim.ROOT_PROJECT_SOLR}:52_digital_index_of_north_american_archaeology_dinaa___*')
